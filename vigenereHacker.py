...
def findRepeatSequenceSpacings(message):
	...
	#that are repeated. Returnd a dict with the keys of the sequence and
	...
	for seqLen in range(3,6):
		for seqStart in range(len(message)- seqLen):
			#Determine what the sequence is, and store it in the seq
...
def getMostCommonFactors(seqFactors):
	...
	#spacings. seqFactors has a value like:{'GFD':[2,3,4,6,9,12,18,23,36,46,69,92,138,207],'ALW':[2,3,4,6,...],...}
	...
	for indexes in itertools.product(range(NUM_MOST_FREQ_LETTERS), repeat = mostLikelyKeyLength):
		...
		if not SILENT_MODE:
			...
			if detectEnglish.isEnglish(decryptedText):
				...
				print()
				print('Enter D for done, or just press Enter to contine hacking:')
				response = input('> ')
...
def hackVigenere(ciphertext):
	...
	#if none of the key lengths we foundusing kasiski Examination worked, 
